...
enemy_die_sound = pygame.mixer.Sound("fd.mp3")  # Sound for enemy defeat
defeat_sound = pygame.mixer.Sound("s.mp3")  # Sound for player defeat
victory_sound = pygame.mixer.Sound("g.mp3")  # Load victory sound
...
start_image = pygame.image.load("j.jpeg")  # Load your start screen image

# Load background images
background1 = pygame.image.load("v.png")  # Replace with your background image
background2 = pygame.image.load("s.png")  # Replace with your background image
background3 = pygame.image.load("f.png")  # Replace with your background image
...
start_image = pygame.transform.scale(start_image, (SCREEN_WIDTH, SCREEN_HEIGHT))  # Scale start image
...
def draw_rounded_rect(surface, color, rect, radius):
    ...
    rounded_rect = pygame.Surface(rect.size, pygame.SRCALPHA)
    pygame.draw.rect(rounded_rect, color, (0, 0, rect.width, rect.height), border_radius=radius)
    surface.blit(rounded_rect, rect.topleft)
...
def start_screen():
    ...
    button_text = button_font.render("Click to Start", True, (255, 255, 255))
    button_width = 300
    button_height = 50
...
def background_selection_screen():
    font = pygame.font.Font(None, 74)
    title_text = font.render("Select Background", True, (0, 0, 0))
    title_rect = title_text.get_rect(center=(SCREEN_WIDTH // 2, 50))
    backgrounds = [background1, background2, background3]
    selected_background = None
    buttons = []
    button_colors = [(3, 155, 229), (67, 160, 71), (121, 85, 72)]  # Light Blue, Dark Green, Light Brown
...
def display_victory_screen(enemies_killed):
    font = pygame.font.Font(None, 74)
    text = font.render("Victory!", True, (2, 48, 32))
    message_font = pygame.font.Font(None, 36)
    message = message_font.render(f"You killed {enemies_killed} enemies!", True, (0, 0, 0))
    next_stage_message = message_font.render("Going to the next stage...", True, (0, 0, 0))
...
def game_over_screen(enemies_killed):
    ...
    message_font = pygame.font.Font(None, 36)
    message = message_font.render(f"You killed {enemies_killed} enemies!", True, (0, 0, 0))
    restart_text = message_font.render("Press R to Restart or Q to Quit", True, (0, 0, 0))
    screen.fill((255, 255, 255))  # White background
...
def main(background):
    ...
    defeated_enemies = 0
    total_enemies_to_defeat = 30  # Enemies to defeat to complete a stage
    running = True
...
if __name__ == "__main__":
    ...
    selected_background = background_selection_screen()  # Show background selection screen
    main(selected_background)  # Start the game with the selected background